...
export default function ERPCard({ item }: ERPCardProps) {
  ...
  return (
    <div
      className={`bg-white p-8 rounded-xl shadow-md transition-transform hover:scale-105 hover:shadow-xl border-l-8 ${statusClass} w-full`}
    >
      <h2 className="text-3xl font-bold text-gray-800 mb-6 break-words">
        {item.name}
      </h2>

      {/* Grid layout for consistent alignment */}
      <div className="grid grid-cols-[170px_1fr] gap-y-6 gap-x-4">
        {/* Status */}
